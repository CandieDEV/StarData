...
let origin = null;
rectangleDisplay.onmousedown = (e) => {
...
function cutImg(e) {
  ...
  const { srcX, srcY, srcW, srcH, destX, destY, destW, destH } = getPosOfCutIMG(
    starDisplay,
    image,
    rect,
  );
...
cutImgBtn.addEventListener("click", (e) => {
  cutImg(e);
});
...
undoIMGbtn.addEventListener("click", (e) => {
  undoAction();
});
...
window.addEventListener("keydown", (e) => {
  ...
  if (evtobj.ctrlKey && evtobj.keyCode == 83) {
    ...
    const dataURL = starDisplay.toDataURL("image/png", 1.0);
